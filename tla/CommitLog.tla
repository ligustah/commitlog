...
(*                                                                         *)
(* Concrete code facts this abstracts (verified against commitlog.go /     *)
(* segment.go): SetHighWatermark never fsyncs; a segment roll seals the    *)
(* old active WITHOUT fsyncing it; checkpointHW fsyncs only the ACTIVE     *)
(* segment and writes ckpt = hw; SyncAll fsyncs every segment then         *)
(* checkpoints. The model collapses this per-segment fsync granularity     *)
(* into one `durable` frontier and takes the CONSERVATIVE (power-loss)     *)
(* view -- so it proves the safety floor. The code additionally recovers   *)
(* written-but-unsynced data after a kill -9 (RecoverTail's forward scan   *)
(* over the OS-cached tail), a STRONGER recovery this model does not       *)
(* assert; here Crash may drop any un-`durable` committed tail.            *)
(*                                                                         *)
(* Invariants checked:                                                     *)
